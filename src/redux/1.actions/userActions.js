...
//GET DATA FROM FAKE API DB.JSON
...
export const qtyCart = (param) => {
    return dispatch => {
      Axios.get(urlApi + "cart?userId=" + param).then(res => {
        dispatch({
          type: "CARTING",
          payload: {
  ...
  export const HistoCart = param => {
    return dispatch => {
      Axios.get(urlApi + "history?userId=" + param).then(res => {
        dispatch({
          type: "HISTORY_SHOP",
          payload: {
            historyLength: res.data.length
          }
        });
      });
    };
